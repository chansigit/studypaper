---
name: paperstudio:reproduce-check
...
Invokes the `reproduce-check` skill from the `paperstudio` plugin.
...
Output: `~/claude-papers/papers/<slug>/reproduce-check.md` with a Summary table, per-dimension sections, and (if any ✗ or ≥3 partials) a "Recommended next steps" section suggesting `/paperstudio:review-round` to convert weaknesses into review.md entries.

The skill backs up an existing `reproduce-check.md` to `<file>.bak.NN` before overwriting.

Pre-requisites: `/paperstudio:study` must have produced the analysis directory + `meta.json`.
