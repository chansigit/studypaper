...
Invoke after `/paperstudio:study` has produced the focal paper's analysis. Each invocation produces one comparison markdown file at `compares/vs-<other-slug>.md`.
...
Verify `$THIS_PAPER_DIR/analysis/00-paper-profile.md` exists. If not, abort: `"Focal paper has no analysis directory. Run /paperstudio:study first."`
...
Set `OTHER_PAPER_DIR=~/claude-papers/papers/<other-paper>`. Verify it exists with `analysis/00-paper-profile.md`. If not, abort: `"<slug> has no analysis directory in ~/claude-papers/papers/. Run /paperstudio:study <slug> first or pass a PDF/URL to auto-study."`
...
```
The other paper has not been studied yet. Auto-studying it now via /paperstudio:study; this may take a few minutes.
```

Invoke `/paperstudio:study` via the Skill tool:

```
Skill(skill: "paperstudio:study-deep", args: "<other-paper> --yes")
```
...
Verify it's a different folder from `THIS_PAPER_DIR` (defensive). If they match, abort: `"Auto-study did not produce a distinct paper folder. Check /paperstudio:study output."`
...
Run /paperstudio:compare again with a different paper to add another comparison.
```
...
- **Auto-study side effect:** When `<other-paper>` is a PDF path or URL, this skill auto-studies the other paper via `/paperstudio:study --yes`. That produces the full Plan 1 outputs for the other paper (12 artifacts), not just the analysis directory. The auto-study is a documented side effect — once-per-paper.
