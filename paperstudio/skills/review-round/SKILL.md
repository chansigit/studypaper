---
name: review-round
description: Use when the user wants to run an adversarial review round on a paper that has already been studied via /paperstudio:study. Solicits objections, dispatches defense+judge sub-Agents, gets the user's final verdict, and updates review.md with accepted weaknesses or questions.
...
Verify:
- `$PAPER_DIR/review.md` exists. If not, abort with: "No review.md found at <path>. Run /paperstudio:study on this paper first."
...
```
<!-- generated: <runtime-iso8601-utc> by review-round-orchestrator (paperstudio v<plugin-version>) -->
...
Run /paperstudio:review-round again any time to layer more rounds. Run /paperstudio:rerun-stage review to discard all rounds and regenerate review.md from analysis (you'll lose the round history's edits).
```
...
- **No idempotence skip:** unlike `study-deep`'s default skip-existing behavior, every invocation of `/paperstudio:review-round` produces fresh rounds. The user is the source of truth for what to argue about.
